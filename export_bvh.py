...
def bvh_save(filename, data):
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(len(parents)):
            if parents[i] == 0:
                t, jseq = bvh_save_joint(
                    f, offsets, order, parents, names, t, i, jseq)
...
def bvh_save_joint(f, offsets, order, parents, names, t, i, jseq):
    ...
    for j in range(len(parents)):
        if parents[j] == i:
            t, jseq = bvh_save_joint(
                f, offsets, order, parents, names, t, j, jseq)
            end_site = False
...
def quat_to_euler(x, order='zyx'):
    ...
    if order == 'zyx':
        return np.concatenate([
            np.arctan2(2.0 * (x0 * x3 + x1 * x2),
                       1.0 - 2.0 * (x2 * x2 + x3 * x3)),
            np.arcsin(np.clip(2.0 * (x0 * x2 - x3 * x1), -1.0, 1.0)),
            np.arctan2(2.0 * (x0 * x1 + x2 * x3),
                       1.0 - 2.0 * (x1 * x1 + x2 * x2)),
        ], axis=-1)
    elif order == 'xzy':
        return np.concatenate([
            np.arctan2(2.0 * (x1 * x0 - x2 * x3), -x1 *
                       x1 + x2 * x2 - x3 * x3 + x0 * x0),
            np.arctan2(2.0 * (x2 * x0 - x1 * x3), x1 *
                       x1 - x2 * x2 - x3 * x3 + x0 * x0),
...
filenames = [f[:-4] for f in os.listdir('F:/OpenHuman/zeroeggs-retarget/fbx') if f.endswith('.fbx')]

for file in filenames:

    input_file = 'F:/OpenHuman/zeroeggs-retarget/fbx/'+file+'.fbx'
    output_file = 'F:/OpenHuman/zeroeggs-retarget/bvh/'+file+'.bvh'
    ...
    bone_global_rotations = quat_from_xform(
        bone_xforms[:, :, :3, :3].transpose([0, 1, 3, 2]))

    bone_local_positions = bone_global_positions.copy()
    bone_local_rotations = bone_global_rotations.copy()

    bone_local_positions[:, 1:] = quat_inv_mul_vec(
        bone_global_rotations[:, parents[1:]], bone_global_positions[:, 1:] - bone_global_positions[:, parents[1:]])
    bone_local_rotations[:, 1:] = quat_abs(quat_inv_mul(
        bone_global_rotations[:, parents[1:]], bone_global_rotations[:, 1:]))
